...
def radius_graph(
    ...
    *,
    ...
    """Dependency-free exact radius candidates using chunked pair distances.

    The implementation is a correctness-oriented PyTorch reference. It avoids a
    full cross-graph pair matrix and bounds temporary memory by ``chunk_size``,
    but arithmetic is quadratic within each graph. Repeated training should
    prepare and cache a graph, or use a future cell-list/Triton backend.
    """
    ...
    if not 0.0 < cutoff_value < float("inf"):
    ...
    if batch is None:
        batch_long = torch.zeros(
            positions.shape[0],
            device=positions.device,
            dtype=torch.long,
        )
        num_graphs = 1 if positions.shape[0] else 0
    else:
        if batch.device != positions.device:
            raise ValueError("batch and positions must share one device")
        num_graphs = _require_contiguous_batch(batch)
        batch_long = batch.to(dtype=torch.long)
    ...
    for graph_id in range(num_graphs):
        node_index = (batch_long == graph_id).nonzero(as_tuple=False).flatten()
        graph_positions = detached.index_select(0, node_index)
        graph_nodes = int(node_index.numel())
        if graph_nodes == 0:
            continue
        for start in range(0, graph_nodes, chunk_size):
            stop = min(start + chunk_size, graph_nodes)
            receiver_position = graph_positions[start:stop]
            squared_distance = (
                receiver_position[:, None, :] - graph_positions[None, :, :]
            ).square().sum(dim=-1)
            valid = squared_distance < cutoff_squared
            if not include_self:
                local_receiver = torch.arange(
                    start,
                    stop,
                    device=positions.device,
                )
                valid[
                    torch.arange(stop - start, device=positions.device),
                    local_receiver,
                ] = False
            if max_neighbors is None:
                receiver_local, sender_local = valid.nonzero(as_tuple=True)
                receivers.append(node_index[start + receiver_local])
                senders.append(node_index[sender_local])
                continue

            k = min(max_neighbors, graph_nodes)
            masked_distance = torch.where(
                valid,
                squared_distance,
                torch.full_like(squared_distance, float("inf")),
            )
            distance, sender_local = torch.topk(
                masked_distance,
                k=k,
                dim=1,
                largest=False,
            ...
            kept = torch.isfinite(distance)
            receiver_local = torch.arange(
                start,
                stop,
                device=positions.device,
            ).unsqueeze(1).expand_as(sender_local)
            receivers.append(node_index[receiver_local[kept]])
            senders.append(node_index[sender_local[kept]])

    if not receivers:
        return torch.empty((2, 0), device=positions.device, dtype=torch.long)
    return torch.stack([torch.cat(receivers), torch.cat(senders)]).to(dtype=torch.long)
