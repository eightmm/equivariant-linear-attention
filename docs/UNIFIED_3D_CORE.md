...
`UnifiedEquivariantAttention` is the canonical generic 3D path. Users choose
the final `output_irreps`; internal angular degree, parity, routing,
normalization, and local/global composition are fixed by the implementation.
...
output = model(node_features, positions, graph)
...
The output declaration is the only representation-level user choice. Width,
depth, head count, rank, and cutoff are resource/capacity hyperparameters.
...
The scalar input and position are

\[
f_i\in\mathbb R^{C_{\rm in}},
\qquad
x_i\in\mathbb R^3.
\]

Optional inputs are polar vectors and even symmetric-traceless tensors:

\[
v_i^{\rm in}\in\mathbb R^{C_{1,\rm in}\times3},
\]

\[
Q_i^{\rm in}\in\mathbb R^{C_{2,\rm in}\times3\times3},
\quad
Q=Q^T,\quad\operatorname{tr}Q=0.
\]
...
The even scalar state is

\[
s_i^{+,0}=W_{\rm in}f_i+e_{\rm role(i)}.
\]
...
T_i^{+,0}
=
W_Q\operatorname{ST5}(Q_i^{\rm in})
+
\tanh(G_Ts_i^{+,0})
\odot\operatorname{ST}(\bar x_i).
\]

The odd scalar, axial vector, and odd tensor states start at zero:

\[
s_i^{-,0}=0,\qquad A_i^{+,0}=0,\qquad U_i^{-,0}=0.
\]

They are generated by chiral local moments and then propagated globally.
